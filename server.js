...
server.post('/users/register', async (req,res) => {
    try {
        ...
        res.status(500).json({ message: 'Szerver hiba: ' + e.message, errors: e.errors });
    }
})
...
server.post('/trips', Auth(), async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Hiba az utazás mentésekor', error: e.message })
    }
})
...
server.get('/trips', Auth(), async (req, res) => {
    try {
        const trips = await dbHandler.table2.findAll({ where: { creator: req.user.username } })
        res.status(200).json(trips)
    } catch (e) {
        res.status(500).json({ message: 'Nem sikerült lekérni az utazásokat', error: e.message })
    }
})
